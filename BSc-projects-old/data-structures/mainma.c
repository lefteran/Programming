...
typos_stoivas sorted, allsorted;
int overflow1,overflow2,overflow3;

dimiourgia(&sorted);
dimiourgia(&allsorted);

    FillSorted(&sorted, &overflow1, 'a');
    if (overflow1)
       printf("H Stoiva sorted1 den xorese ola ta stoixeia apo aristera\n");
    /* EmptySorted(&Sorted1); Energopoihste mono gia dokimh. Grafei kai Adeiazei! Stoiva */
    
    FillSorted(&sorted, &overflow2, 'd');
    if (overflow2)
       printf("H Stoiva sorted den xorese ola ta stoixeia apo deksia\n");
    /* EmptySorted(&Sorted2); Energopoihste mono gia dokimh. Grafei kai Adeiazei! Stoiva */
	...
	*overflow=0;
     ...
     {     //printf("in loop\n");
	   ...
	   paratirisi(*In, &Max2, 'd');           
	      ...
	      othisi(Out, Max2, overflow, 'a');    // eisafvgh mefalyteroy sthn Out
              if (*overflow)
